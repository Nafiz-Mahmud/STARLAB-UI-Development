export default function Featured() {
    ...
    return (
        <div className="featured bg-green-600 p-[2rem] xl:p-[4rem]">
            ...
            <div className="brands flex items-center justify-around mt-[3rem]">
                {
                    brands.map((brand, index) => {
                        return (
                            <h1 className="brand text-white font-bold text-[.8rem] sm:text-[1rem] md:text-[1.5rem] xl:text-[2rem]" key={index}>{brand}</h1>
                        )
                    })
                }
            </div>
